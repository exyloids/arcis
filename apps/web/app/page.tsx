...
import { Apple, Armchair, BatteryCharging, Beef, Bike, BookOpen, Briefcase, Building2, BusFront, Cake, CalendarDays, Car, CircleDollarSign, Coffee, CreditCard, Droplets, Dumbbell, FileText, Flame, Footprints, Fuel, Gamepad2, Gem, Gift, HeartPulse, House, Landmark, Milk, Monitor, MoreHorizontal, Music2, Package, ParkingCircle, PartyPopper, PenLine, Phone, Pill, Pizza, Plane, Scissors, ShieldCheck, Shirt, ShoppingBasket, Sparkles, Sprout, Stethoscope, TestTube, Ticket, ToyBrick, TrainFront, Truck, Trophy, Tv, Users, Utensils, Wifi, Wine, Wrench, Zap, type LucideIcon } from "lucide-react";

const API_URL = process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:8000";
const navigation = [
  ["home", "Home"], ["transactions", "Transactions"], ["spending", "Spending"], ["budgets", "Budgets"], ["recurring", "Recurring"], ["accounts", "Accounts"],
  ["cards", "Cards"], ["imports", "Imports"], ["documents", "Documents"], ["mailboxes", "Mailboxes"], ["privacy", "Privacy"],
] as const;
...
type NotificationItem = { id: string; notification_kind: string; title: string; body: string; state: "unread" | "read" | "dismissed"; due_at: string | null };
type DocumentItem = { id: string; kind: string; detected_mime_type: string | null; byte_size: number | null; lifecycle_state: string; created_at: string; deleted_at: string | null; purge_after: string | null; can_restore: boolean; import_id: string | null; filename: string | null; import_state: string | null; account_name: string | null; mailbox_email: string | null; parser_state: string | null; review_reason: string | null };
...
export default function HomePage() {
  ...
  async function refresh(selectedPeriod: ReportingPeriod = reportingPeriod) {
    ...
    if (ledgerCategoryId) ledgerParams.set("category_id", ledgerCategoryId);
    if (search.trim()) ledgerParams.set("q", search.trim());
    const [nextAccounts, nextCategories, transactionPage, nextReport, nextBalanceSummary, nextImports, nextMailboxes, nextCandidates, nextAttachments, nextReviews, nextRecurringPayments, nextMonthlyInsights, nextSpendingSummary, nextBudgets, nextCardStatements, nextNotifications, nextDocuments, nextPrivacyInventory] = await Promise.all([
    ...
    setDocuments(nextDocuments); setPrivacyInventory(nextPrivacyInventory);
  ...
  }, [activeView, ledgerAccountType, ledgerAccountId, ledgerCategoryId, reportingPeriod, search]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (!selectedTransaction && !taggingTransaction && !evidence) return;
    ...
    const handleDialogKeyboard = (event: KeyboardEvent) => {
      if (event.key === "Escape") {
        if (taggingTransaction) setTaggingTransaction(null);
  ...
  }, [selectedTransaction, taggingTransaction, evidence]);
  useEffect(() => {
    if (selectedTransaction || taggingTransaction || evidence || !dialogReturnFocus.current) return;
  ...
  async function redactDocument(id: string) { if (!window.confirm("Delete the stored source file? Ledger entries and provenance metadata will remain.")) return; try { await request(`/api/v1/documents/${id}/content`, { method: "DELETE" }); setMessage("Stored document content deleted. Ledger provenance was retained."); await refresh(); } catch (error) { setMessage((error as Error).message); } }
  async function restoreDocument(id: string) { try { await request(`/api/v1/documents/${id}/restore`, { method: "POST" }); setMessage("Document content restored from the recovery area."); await refresh(); } catch (error) { setMessage((error as Error).message); } }
  ...
  return <main className="app-shell">
    ...
    <div className="main-content">
      <header className="topbar"><div><p className="eyebrow">ARCIS FINANCE</p><h1>{activeView === "home" ? greeting() : viewTitle(activeView)}</h1><p className="subtitle">{activeView === "home" ? "Your money, clearly organised." : viewSubtitle(activeView)}</p></div><div className="top-actions">{activeView === "home" && <select className="global-period" value={reportingPeriod} onChange={(event) => void changeReportingPeriod(event.target.value as ReportingPeriod)} aria-label="Reporting period">{reportingPeriods.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>}<button className="icon-button" onClick={() => void refresh()} aria-label="Refresh data">↻</button><button className="profile" onClick={() => setActiveView("mailboxes")} aria-label="Open mailbox settings">A</button></div></header>
...
function NotificationList({ notifications, onDismiss }: { notifications: NotificationItem[]; onDismiss: (id: string) => void }) { if (!notifications.length) return <p className="muted">No upcoming or overdue card reminders.</p>; return <div className="notification-list">{notifications.map((notification) => <article key={notification.id}><span className={notification.notification_kind.includes("overdue") ? "notification-mark overdue" : "notification-mark"}>!</span><div><strong>{notification.title}</strong><small>{notification.body}</small></div><button className="secondary mini" onClick={() => onDismiss(notification.id)}>Dismiss</button></article>)}</div>; }
...
function SpendingCategoryList({ summary, selectedCategoryId, onSelect }: { summary: SpendingSummary | null; selectedCategoryId: string; onSelect: (id: string) => void }) { const categories = summary?.categories ?? []; if (!categories.length) return <p className="muted">No categorised spending in this period yet.</p>; return <div className="spending-category-list">{categories.map((category, index) => <button key={category.category} className={category.category_id === selectedCategoryId ? "selected" : ""} disabled={!category.category_id} onClick={() => category.category_id && onSelect(category.category_id)}><i style={{ background: spendingColours[index % spendingColours.length] }} /><span><strong>{category.category}</strong><small>{Number(category.percentage).toFixed(1)}%</small></span><b>{formatAmount(category.amount)}</b></button>)}</div>; }
...
function DocumentVault({ documents, onOpenImport, onRedact, onRestore }: { documents: DocumentItem[]; onOpenImport: (id: string) => void; onRedact: (id: string) => void; onRestore: (id: string) => void }) {
  if (!documents.length) return <section className="panel empty-vault"><FileText size={32} /><h3>No stored documents</h3><p className="muted">Imported statements and Gmail source artifacts will appear here.</p></section>;
  return <section className="document-grid">{documents.map((document) => { const title = document.filename ?? (document.kind === "gmail_attachment" ? "Gmail statement attachment" : document.kind === "gmail_message" ? "Gmail transaction email" : "Source document"); const source = document.account_name ?? document.mailbox_email ?? "Arcis source"; return <article className="document-card" key={document.id}><span className="document-icon"><FileText size={22} /></span><div className="document-copy"><strong>{title}</strong><small>{source} · {new Date(document.created_at).toLocaleString()}</small><span><em className={`state ${document.lifecycle_state}`}>{document.lifecycle_state}</em>{document.import_state && <em className={`state ${document.import_state}`}>{document.import_state.replaceAll("_", " ")}</em>}{document.parser_state && <em className={`state ${document.parser_state}`}>{document.parser_state.replaceAll("_", " ")}</em>}</span>{document.review_reason && <small>{document.review_reason}</small>}{document.purge_after && <small>Recovery available until {new Date(document.purge_after).toLocaleDateString()}</small>}</div><div className="document-meta"><b>{formatFileSize(document.byte_size)}</b>{document.import_id && <button className="secondary mini" onClick={() => onOpenImport(document.import_id!)}>Open import</button>}{document.lifecycle_state === "active" && <button className="secondary mini danger" onClick={() => onRedact(document.id)}>Delete file</button>}{document.can_restore && <button className="secondary mini" onClick={() => onRestore(document.id)}>Restore</button>}</div></article>; })}</section>;
}
...
function viewTitle(view: View) { return ({ transactions: "Transactions", spending: "Spending", budgets: "Budgets", recurring: "Recurring", accounts: "Accounts", cards: "Credit cards", imports: "Imports", documents: "Documents", mailboxes: "Mailboxes", privacy: "Privacy", home: "Home" })[view]; }
function viewSubtitle(view: View) { return ({ transactions: "Search, review, and categorise your ledger.", spending: "Understand where your money goes over time.", budgets: "Plan monthly category spending and track progress.", recurring: "Review recurring payments and subscription commitments.", accounts: "Your savings accounts and recorded balances.", cards: "Card activity is separate from your bank balance.", imports: "Review statement data before it reaches the ledger.", documents: "Review and control retained financial source files.", mailboxes: "Connect and manage your Gmail transaction sources.", privacy: "Export your data and control source-file retention.", home: "Your money, clearly organised." })[view]; }
function navIcon(view: View) { return ({ home: "⌂", transactions: "↔", spending: "◔", budgets: "◎", recurring: "↻", accounts: "⌑", cards: "▣", imports: "⇧", documents: "▤", mailboxes: "✉", privacy: "◇" })[view]; }
